...
//randomly allocate capabilities while respecting the constraints provided in the line below.
	...
	for(i=0;i<playeramount;i++)
	{
		...
		{
			case 1: //movement
				i = playermove(i,&player[i].currentrow, &player[i].currentcol);
				
				if(check == i) //if there's an error, i returns as "i-1"
				{
					updatestats(**slot[player[i].currentrow][player[i].currentcol].slottype[10], &player[i].strength, &player[i].dexterity, &player[i].smartness, &player[i].magic);
				}
				break;
