...
// SECTION B: RANDOM ALLOCATION OF PLAYER CAPABILITIES
	...
	for (i=0; i<n; i++){
		...
		{
			case 1: player[i].type = human();
				break; 	//HUMAN

			case 2: player[i].type =ogre();
				break; //OGRE
			case 3: player[i].type =elf();
				break;//ELF
			case 4: player[i].type = wizard();
				break;//WIZARD
...
/*{ //SECTION B OF CODE MOVED TO HEADER. KEPT ORIGINAL COMMENTED OUT IN HERE
	...
	srand ( time(NULL)); 
 {
	...
	printplayersummary(); //function to print out player summary 
